% Universidade Federal de Santa Catarina
% Centro Tecnológico
% Departamento de Automação e Sistemas
% Autores: Alex Amadeu Cani
%          Gabriel Paiva
%          Iago de Oliveira Silvestre
%          Luís Artur Kretzer Tavares Sobral
% Ano/Semestre: 2017-2
% 
% Disciplina: Projeto Integrador (DAS5104)
% Kit Didático para Controle

function hf = buildInterface(tag)
scz = get(0, 'ScreenSize'); % Tamanho da tela

% Figura principal
hf = figure('Tag', tag,...
	'Toolbar','none',...
	'MenuBar','none',...
	'Units','pixels',...
	'OuterPosition', [0 0 scz(3)*0.98 scz(4)*0.87],...
	'Units','normalized',...
	'Resize','off',...
	'IntegerHandle','off',...
    'NumberTitle','off',...
    'HandleVisibility','callback',...
    'Name','Interface',...
    'CloseRequestFcn',@cb_closeMainWindow,...
    'Visible','off');

%% Cria o painel Controle
hcontrole = uipanel('Parent', hf,...
    'Units','normalized',...
    'Position',[0.005 0.52 0.16 0.475],...
    'Title','Controle',...
    'BackgroundColor',get(hf,'Color'),...
    'HandleVisibility','callback',...
    'Tag','painelControlador');

% Texto Controlador
uicontrol('Parent',hcontrole,...
    'Style','text',...
    'Units','normalized',...
    'FontWeight', 'bold',...
    'Position',[0 0.9 1 0.06],...
    'BackgroundColor',get(hf,'Color'),...
    'String','Tipo de Controlador',...
    'HandleVisibility','callback');

% Buttongroup p escolha do tipo de controlador
hbgcontrolador = uibuttongroup('Parent',hcontrole,...
    'Units','normalized',...
    'Position',[0.03 0.83 0.94 0.07],...
    'BackgroundColor',get(hf,'Color'),...
    'HandleVisibility','callback',...
    'BorderType', 'none',...
    'SelectionChangedFcn', @cb_controllerTypeChanged);%

uicontrol(hbgcontrolador, 'Style','radiobutton',...
    'Units','normalized',...
    'Position',[0 0 0.5 1],...
    'String','Pré-definido',...
    'UserData', 'pre',...
    'Backgroundcolor',get(hf,'Color'),...
    'HandleVisibility','callback');

uicontrol(hbgcontrolador, 'Style','radiobutton',...
    'Units','normalized',...
    'Position',[0.5 0 0.5 1],...
    'String','Personalizado',...
    'UserData', 'custom',...
    'Backgroundcolor',get(hf,'Color'),...
    'HandleVisibility','callback');

%% Cria o painel Simulação
hsim = uipanel('Parent', hf,...
    'Units','normalized',...
    'Position',[0.005 0.30 0.16 0.215],...
    'Title','Simulação',...
    'BackgroundColor',get(hf,'Color'),...
    'HandleVisibility','callback',...
    'Tag','painelSimulacao');

% Botões
textos = {'Inicia','Para'}; % Textos dos botões (Botão para é o mesmo botão do continua depois)
positions = [0.605 0.315];
tags = {'bt_iniciaSim','bt_paraContinuaSim'};
callbacks = {@cb_iniciaSim, @cb_paraContinuaSim};
enabled ={'On','off'};
for index = 1:length(textos)
    uicontrol('Parent',hsim,...
        'Style','pushbutton',...
        'Units','normalized',...
        'Position',[0.15 positions(index) 0.7 0.22],...
        'BackgroundColor',get(hf,'Color'),...
        'String',textos{index},...
        'Enable',enabled{index},...
        'HandleVisibility','callback',...
        'Callback',callbacks{index},...
        'Tag',tags{index});
end

% Checkbox tempo real ou nao
check = uicontrol(hsim,...
    'Style','checkbox',...
    'Units','normalized',...
    'Position',[0.15 0.14 1 0.15],...
    'BackgroundColor',get(hf,'Color'),...
    'String','Simular em tempo real',...
    'Value', 1,...
    'HandleVisibility','callback');

check.Position = [check.Position(1) check.Position(2) check.Position(3) check.Extent(4)+0.001];

%% Cria o painel da Planta Física
hplanta = uipanel('Parent', hf,...
    'Units','normalized',...
    'Position',[0.005 0.005 0.16 0.29],...
    'Title','Planta',...
    'BackgroundColor',get(hf,'Color'),...
    'HandleVisibility','callback',...
    'Tag','painelSimulacao');

%% Cria o painel da visualização 3D

h3d = uipanel('Parent', hf,...
    'Units','normalized',...
    'Position',[0.17 0.52 0.825 0.475],...
    'Title','Visualização',...
    'BackgroundColor',get(hf,'Color'),...
    'HandleVisibility','callback');

% Cria os eixos da visualização
ha_visu = axes('Parent',h3d,...
    'HandleVisibility','callback',...
    'Unit','normalized',...
    'Position',[0.04 0.1 0.95 0.83],...
    'Xlim',[-5 105],...
    'YLim',[-45 15],...
    'XMinorTick', 'on',...
    'DataAspectRatioMode', 'auto');
xlabel(ha_visu,'X [cm]');
ylabel(ha_visu,'Y [cm]');
title(ha_visu,'Visualização');
grid(ha_visu,'on');
box(ha_visu,'on');

%% Painel com a visualização das variáveis (não tem um 'Panel')
alturas = 0.105;

% Posição do carrinho
ha_posX = axes('Parent',hf,...
    'HandleVisibility','callback',...
    'Unit','normalized',...
    'Position',[0.195 0.39 0.3775 alturas],...
    'Xlim',[0 10],...
    'YLim',[-0.7 1],...
    'TitleFontSizeMultiplier', 0.9,...
    'LabelFontSizeMultiplier', 0.9);
title(ha_posX,'Posição do Carrinho');
ylabel(ha_posX,'Posição [cm]');
xlabel(ha_posX, 'Tempo [s]');
grid(ha_posX,'on');
box(ha_posX,'on');

% Posição do pêndulo
ha_posTheta = axes('Parent',hf,...
    'HandleVisibility','callback',...
    'Unit','normalized',...
    'Position',[0.195 0.375 0.3775 alturas],...
    'Xlim',[0 10],...
    'YLim',[-0.7 1],...
    'TitleFontSizeMultiplier', 0.9,...
    'LabelFontSizeMultiplier', 0.9);
title(ha_posTheta,'Posição do Pêndulo');
ylabel(ha_posTheta,'Posição [rad]');
xlabel(ha_posTheta, 'Tempo [s]');
grid(ha_posTheta,'on');
box(ha_posTheta,'on');

% Atuação
ha_atuacao = axes('Parent',hf,...
    'HandleVisibility','callback',...
    'Unit','normalized',...
    'Position',[0.195 0.045 0.3775 alturas],...
    'Xlim',[0 10],...
    'YLim',[-0.7 1],...
    'TitleFontSizeMultiplier', 0.9,...
    'LabelFontSizeMultiplier', 0.9);
title(ha_atuacao,'Atuação');
ylabel(ha_atuacao,'Tensão [% V]');
xlabel(ha_atuacao, 'Tempo [s]');
grid(ha_atuacao,'on');
box(ha_atuacao,'on');

align([ha_posX ha_posTheta ha_atuacao], 'None', 'Distribute')


% Velocidade do carrinho
ha_velX = axes('Parent',hf,...
    'HandleVisibility','callback',...
    'Unit','normalized',...
    'Position',[0.6125 ha_posX.Position(2) 0.3775 alturas],...
    'Xlim',[0 10],...
    'YLim',[-0.7 1],...
    'TitleFontSizeMultiplier', 0.9,...
    'LabelFontSizeMultiplier', 0.9);
title(ha_velX,'Velocidade do Carrinho');
ylabel(ha_velX,'Velocidade [cm/s]');
xlabel(ha_velX, 'Tempo [s]');
grid(ha_velX,'on');
box(ha_velX,'on');

% Velocidade do pêndulo
ha_velTheta = axes('Parent',hf,...
    'HandleVisibility','callback',...
    'Unit','normalized',...
    'Position',[0.6125 ha_posTheta.Position(2) 0.3775 alturas],...
    'Xlim',[0 10],...
    'YLim',[-0.7 1],...
    'TitleFontSizeMultiplier', 0.9,...
    'LabelFontSizeMultiplier', 0.9);
title(ha_velTheta,'Velocidade do Pêndulo');
ylabel(ha_velTheta,'Velocidade [rad/s]');
xlabel(ha_velTheta, 'Tempo [s]');
grid(ha_velTheta,'on');
box(ha_velTheta,'on');

%% Painel de controle das exibições (exportar, hold, etc)
% Botão exportar selecionados
uicontrol('Parent',hf,...
    'Style','pushbutton',...
    'Units','normalized',...
    'Position',[0.6125 0.105 0.07 0.05],...
    'BackgroundColor',get(hf,'Color'),...
    'String','Exportar Selecionados',...
    'Enable','on',...
    'HandleVisibility','callback',...
    'Callback','cb_exportarSelecao',...
    'Tag','bt_exportaSelecao');

% Checkboxes com as opções
textos = {'Posição do Carrinho','Posição do Pêndulo', 'Velocidade do Carrinho','Velocidade do Pêndulo','Atuação'}; % Textos dos botões (Botão para é o mesmo botão do continua depois)
largura = 0.07;
positions = [0 1 2 0 1]*(largura+0.005) + ones(1,5)*0.69;
altura_offset = 0.135;
altura = 0.015;
alturas = [altura_offset altura_offset altura_offset altura_offset-altura-0.01 altura_offset-altura-0.01];
tags = {'check_posCar','check_velCar','check_posPend','check_velPend','check_Atuacao'};
for index = 1:length(textos)
uicontrol(hf,...
    'Style','checkbox',...
    'Units','normalized',...
    'Position',[positions(index) alturas(index) largura altura],...
    'BackgroundColor',get(hf,'Color'),...
    'String',textos{index},...
    'Value', 1,...
    'HandleVisibility','callback',...
    'Tag',tags{index});
end

% Texto caixinha do nome
texto = uicontrol(hf,...
    'Style','text',...
    'Units','normalized',...
    'Position',[0.6125 0.08 0.1 0.015],...
    'BackgroundColor',get(hf,'Color'),...
    'String','Exportar como:' ,...
    'HorizontalAlignment', 'left',...
    'HandleVisibility','callback');

% Caixinha com o nome
uicontrol(hf,...
    'Style','edit',...
    'Units','normalized',...
    'Position',[0.613+texto.Extent(3) 0.0775 0.15 0.02],...
    'BackgroundColor',get(hf,'Color'),...
    'String','resultados' ,...
    'HorizontalAlignment', 'left',...
    'HandleVisibility','callback');

% Texto manter as ultimas simulações
texto = uicontrol(hf,...
    'Style','text',...
    'Units','normalized',...
    'Position',[0.6125 0.03 0.1 0.015],...
    'BackgroundColor',get(hf,'Color'),...
    'String','Manter na tela as últimas ' ,...
    'HorizontalAlignment', 'left',...
    'HandleVisibility','callback');
texto.FontSize = texto.FontSize*1.4;

% Caixinha com o numero de simulações
caixa = uicontrol(hf,...
    'Style','edit',...
    'Units','normalized',...
    'Position',[0.613+texto.Extent(3) 0.027 0.015 0.02],...
    'BackgroundColor',get(hf,'Color'),...
    'String', '1',...
    'HorizontalAlignment', 'center',...
    'HandleVisibility','callback');

% Resto do texto
texto = uicontrol(hf,...
    'Style','text',...
    'Units','normalized',...
    'Position',[caixa.Position(1)+0.02 0.03 0.1 0.015],...
    'BackgroundColor',get(hf,'Color'),...
    'String','curvas. ' ,...
    'HorizontalAlignment', 'left',...
    'HandleVisibility','callback');
texto.FontSize = texto.FontSize*1.4;

% Botão Limpar tudo
uicontrol('Parent',hf,...
    'Style','pushbutton',...
    'Units','normalized',...
    'Position',[0.755 0.012 0.07 0.05],...
    'BackgroundColor',get(hf,'Color'),...
    'String','Limpar Tudo',...
    'Enable','on',...
    'HandleVisibility','callback',...
    'Callback','cb_cleanAll',...
    'Tag','bt_cleanAll');
%%
% Armazena todos os dados em hf
% guidata(hf, ad);

% Move para o centro e exibe a tela
movegui(hf, 'center');
set(hf, 'Visible', 'on');